...
extension CassandraClient {
    ...
    ///
    /// The context binds encryption to a specific column (via KEK derivation)
    /// and row (via DEK derivation). Renaming keyspace, table, or column after
    /// data has been encrypted will make that data permanently unreadable.
    public struct EncryptionContext {
        public let keyspace: String
        public let table: String
        public let column: String
        /// Full primary key bytes (partition key + clustering columns) for value columns.
        /// Partition key bytes only for partition-key columns.
...
extension CassandraClient {
    /// Internal cache for derived KEKs and storage for master key material.
    ...
    struct KeysHolder {
        ...
        /// Derive a per-column KEK via HKDF, or return it from cache if already derived.
        /// The cache key is "keyName:contextString" (e.g. "key-2025:prod.users.ssn").
        ...
        /// Derive a per-row DEK from the KEK and primary key bytes.
...
extension CassandraClient {
    ...
    public final class Encryptor {
        static let envelopeVersion: UInt8 = 0x01
        static let algorithmDeterministicDEK: UInt8 = 0x02
        ...
        public func setKeyMap(_ newKeyMap: [String: Data]) throws {
        ...
        internal func encrypt(_ data: Data, context: EncryptionContext) throws -> Data {
            self.lock.lock()
            let keyName = self.currentKeyName
            let dek: SymmetricKey
            do {
                dek = try self.keysHolder.deriveDEK(
                    keyName: keyName,
            ...
            envelope.append(Self.algorithmDeterministicDEK)
        ...
        internal func decrypt(_ envelope: Data, context: EncryptionContext) throws -> Data {
            ...
            offset += 1
            guard algorithm == Self.algorithmDeterministicDEK else {
            ...
            // Derive DEK using the key name from the envelope, not currentKeyName
            let dek: SymmetricKey
            self.lock.lock()
            do {
                dek = try self.keysHolder.deriveDEK(
        ...
        /// Validate a key name and its secret data (size must be 32 bytes).
