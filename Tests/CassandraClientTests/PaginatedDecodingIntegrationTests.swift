...
final class PaginatedDecodingIntegrationTests: XCTestCase {
    ...
    /// The bound values for one fixture row. `sending` because `run` takes `parameters` that way: the
    /// futures below are collected and awaited together, so each array has to be reachable from nowhere
    /// but the call it is handed to.
    private static func insertParameters(
        index: Int,
        nullPayloadAt: Int?
    ) -> sending [CassandraClient.Statement.Value] {
        [
